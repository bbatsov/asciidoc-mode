...
* Fontify superscript (`^text^`) and subscript (`~text~`): the inner text is shrunk and raised or lowered via a `display` property, tunable with `asciidoc-superscript-raise` and `asciidoc-subscript-raise`.
* Make cross-references (`<<id>>`, `xref:`), links, and `link:`/URL macros clickable. `RET` or a `mouse-1`/`mouse-2` click follows the reference (the same action as `C-c C-o`), and references highlight on hover.
* Add an `xref` backend over AsciiDoc anchors and section auto-ids: `M-.` jumps to a definition, `M-,` returns, and `M-?` (`xref-find-references`) lists references. Section ids are derived the way Asciidoctor does, so cross-references resolve to titled sections, not just explicit `[[id]]` anchors.
* Complete cross-reference ids after `<<` / `xref:` via `completion-at-point`, plus attribute names inside `{`, `include::` file paths, and `[source,LANG]` language names in their respective contexts.
* Resolve cross-file `xref:` targets, including Antora component layouts and relative file paths, so following and completion work across a multi-file document set.
* Add an Asciidoctor-backed Flymake checker that reports the parser's errors and warnings inline. It registers automatically, so enabling `flymake-mode` is enough, and it checks unsaved buffers by feeding them to Asciidoctor over its standard input.
* Highlight CSV (`,===`) and DSV (`:===`) tables, with inline markup fontified inside their cells.
* Use dedicated faces for document attributes (`asciidoc-metadata-key-face` / `asciidoc-metadata-value-face`), URL and email targets (`asciidoc-url-face`), and footnotes (`asciidoc-footnote-marker-face` / `asciidoc-footnote-text-face`).
* Integrate with `flyspell`: skip links, cross-references, anchors, and inline code so only prose is spell-checked.
